...
export class SearchResultComponent implements OnInit, OnChanges {
	...
	public getArtistsByName(): void {
		this.searchService.getArtists(this.searchString, this.renderTreshold, this.index, this.filterList ? this.filterList.map(array => array[0]).join(",") : "none", this.filterList ? this.filterList.map(array => array[1]).join(",") : "none", this.sort ? this.sort : "none", this.sortType ? this.sortType : "ascending").subscribe(artists => {
